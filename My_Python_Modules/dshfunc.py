...
def my_read_excel(
    ...
    if print_info is True:
        ...
        print(f"\nRead {file_name} into DataFrame: Done!\n")
    return df2
...
def my_describe(
    ...
    without: list = None,
    cat_cutoff: int = 12,
    print_data_sample=False or True or int,
) -> pd.DataFrame:
    """
    ...
    """
    df = (
        df1.copy().drop(columns=without, inplace=False)
    ...
    if not isinstance(print_data_sample, bool):
        display(df.sample(print_data_sample))
    elif print_data_sample is True:
        display(df)
    return des
...
def my_check_dup(
    df: pd.DataFrame,
    kind=None or "with" or "without",
    subset_col: list = "all" or None,
    ...
    if kind == "with":
        if subset_col == "all":
            print(
                "\nThe number of duplicate rows within the entire dataset is ",
            ...
            print(
                f"The number of duplicate rows within the entire {subset_col} dataset is ",
        ...
        if isinstance(subset_col, list):
            ...
            print(
                f"Except for column {subset_col}, the number of duplicate rows within the dataset is ",
                df.duplicated(
...
def my_handle_dup_and_chk(df: pd.DataFrame, dup_column: list, handling_dict: dict):
    ...
    print(f"The dataset contains {len(df_2)} rows after handling duplicates.")
    return df_2
...
def my_filter_and_update(
    df: pd.DataFrame,
    filter_dict=None or dict,
    filter_kind="and" or "or",
    new_filtered_df=True,
    update_dict=None or dict,
    drop_rows=False,
    index_list=None or list,
) -> pd.DataFrame:
    """
...
def my_sql_groupby_agg_having(
    ...
    multi_level_index=True or False or list,
    ...
    Args:\n
    1. Filter df by multi-columns or index list before grouping\n
        where_dict = {\n
            "col_name_1": [[val1], op1, kind, op2, [val2], resetindex]\n
            "price": [[95], "<=", "and", ">", [5], False],\n
            ("price", "month"): [[386.25], "==", None, None, None, False]\n
            "fund_c": [[40000, 50000], "==", "None, None, None, False],\n
            }\n
    2. Grouping and agg\n
        agg_dict = {
    ...
    8. total_row = False or True, normalize = None or 'row'\n
    """
    ...
    # reset index level
    if isinstance(multi_level_index, list):
        df1.reset_index(level=multi_level_index, inplace=True)
    # flatten row index
    elif multi_level_index is False:
        df1.index = [re.compile(r"[\(\)\']+").sub("", str(i)) for i in df1.index.values]

    # Filter df by condition or by index list after grouping (having in SQL)
...
def my_sql_groupby_having_win_func(
    ...
    sort_idx: dict = None,
    total_row: bool = False,
    normalize=None or "row",
) -> pd.DataFrame:
    """
    Function for grouping by cols or different levels and perform multi-transforming on other cols,...\n
    Args:\n
    1. Filter df by multi-columns or index list before grouping\n
        where_dict = {\n
            "col_name_1": [[val1], op1, kind, op2, [val2], resetindex]\n
    ...
    2. Group by cols or by index and transform\n
        col_to_trans_and_func =
            {
            "price":["mean", "new_col_name1"],\n
            "cost": [lambda x: x.max() - x.min(), "new_col_name2"],
                    }
    3. Filter df by multi-columns or by index list after grouping\n
        having = {
            "price_median": [[95], ">=", None, None, None, False],\n
            ("price_per_month", "sum"): [[386.25], "==", None, None, None, False]\n
            }# Multiindex column\n
...
def my_pivot_agg_filter(
    ...
    col_to_col=list | str,
    agg_dict_or_list=dict or str or list,
    fillna=None,
    # margins=["All", False],
    group_dropna=False,
    multi_level_col=False,
    reset_idx_lv: list = False,
    ...
    Agrs:\n
        1. Filter df before pivot and agg, by multi-columns with "and" | "or" condition between each column \n
        filter_dict = {\n
            "col_name_1": [[val1], op1, kind, op2, [val2], resetindex]\n
            "price": [[95], "<=", "and", ">", [5], False],\n
            ("price", "month"): [[386.25], "==", None, None, None, False]\n
        ...
        filter_kind:\n
            "and" --> filter col_name_1 and col_name_2 and ....\n
            "or" --> filter col_name_1 or col_name_2,...\n
        2. Pivot and agg\n
        agg_dict_or_list = {\n
            "price": ["mean", "median", "count"], #Use standard functions\n
            "price": custom_func, #Or use custom functions\n
        ...
        If agg_dict_or_list is a list of columns, perform pivot func without agg, and agg_dict_or_list is values in pivot func.\n
        3. Filter df by multi-columns after pivot and agg\n
        having_dict ={\n
            "col_name_1": [[val1], op1, kind, op2, [val2], resetindex]\n
            ("price_per_month", "sum"): [[386.25], "==", None, None, None, False]\n
            ("price", "month"): [[386.25], "==", None, None, None, False]\n
    ...
    df1 = (
        ...
        else my_filter_df_by_multi_col(df.copy(), filter_dict, kind=filter_kind)
    )
    # Pivot without agg
    df1 = (
        df1.pivot(index=col_to_index, columns=col_to_col, values=agg_dict_or_list)
        if isinstance(agg_dict_or_list, list)
        else df1
    )
    ...
    # Filter df by condition or by index list after agg
    if having is not None:
        df1 = (
            my_filter_df_by_multi_col(df1, having, kind=having_kind)
            if isinstance(having, dict)
...
def my_string_handling(
    df: pd.DataFrame,
    regex_to_replace=None or dict,
    to_split=None or dict,
    to_join=None or dict,
    changecase=None or dict,
    strip_char=None or dict,
    strip_accents=False,
    convert_numeric_string=None or list,
) -> pd.Series | list:
    """
    Ex1:\n
    ...
    if convert_numeric_string is not None:
        for ser in convert_numeric_string:
            if ser != "all":
                df2[ser] = pd.to_numeric(df2[ser])
            else:
                for ser2 in df2.columns:
                    df2[ser2] = pd.to_numeric(df2[ser2])
    print(
        my_check_category_data(
            df2,
            count=False,
            normalize=False,
            num_to_cat_cutoff=12,
        )
    )
    return df2
